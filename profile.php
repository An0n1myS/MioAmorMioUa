						...
						<div class="panel__open form-panel">
							...
							<ul class="form-panel__list">
								...
								<?php else: ?>
									...
									<li>
										<a class="form-panel__link" href="" id="auth-btn"><?= $_COOKIE['user']  ?></a>
									</li>
									<li>
			...
			<div class="profile">
				...
				<form action="updateProfile.php" method="post">
					<ul class="info-view">
						...
						<li>
							<span>Имя пользователя: <?= $user['name'] ?> </span> <input id="name" name="name" type="text">
						</li>
						<li>
							<span>Адрес электронной почты: <?= $user['mail'] ?> </span> <input id="mail" name="mail" type="email">
						</li>
						...
						<? else: ?>
							<?  	
								...
								if ($result->num_rows > 0) {
									...
									echo "<tr><td>User login</td><td>User name</td><td>User mail</td><td>User status</td><td>Change user status</td><td>Upload</td></tr>";
								    // output data of each row
								    while($row = $result->fetch_assoc()) {
								    	echo "<tr><td>" . $row["login"]. "</td><td>" . $row["name"]. "</td><td>" . $row["mail"]. "</td><td>" . $row["status"]. "</td><td><select id='status' ><option>User</option><option>admin</option><option>moderator</option></select></td><td><button type='submit'>Обновить данные</button></td></tr>";
								    }
								    echo "</table>";
								} else {
								    echo "0 results";
